...
const Login = ({ classes, setNewUser }) => {
  ...
  const handleSubmit = async (event, tokenAuth, client) => {
    ...
    document.cookie =`authToken=${res.data.tokenAuth.token}; expires=Fri, 31 Dec 9999 23:59:59 GMT`;
          ...
          return (
            <form onSubmit={event => handleSubmit(event, tokenAuth, client)} className={classes.form}>
              ...
              <Button
                ...
                color="primary">{loading ? "Logging in..." :"Login"}</Button>
              <Button
                color="secondary"
                variant="outlined"
                onClick={() => setNewUser(true)}
                fullWidth>New user?</Button>
