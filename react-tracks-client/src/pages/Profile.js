...
const Profile = ({ classes, match }) => {
  ...
  return (<Query query={USER_INFO_QUERY} variables={{userId: id}}>
    {({data, loading, error}) => {
      ...
      return (<div>
        ...
        <Paper elevation={1} className={classes.paper}>
          ...
          {data.user.trackSet.map(track => (
            <div key={track.id}>
              <Typography>{track.title} • {track.likeSet.length}</Typography>
        ...
        <Paper elevation={1} className={classes.paper}>
          ...
          {data.user.likeSet.map(({track}) => (
            <div key={track.id}>
              <Typography >
                {track.title} • {track.likeSet.length} • {track.postedBy.username}
              </Typography>
              <AudioPlayer url={track.url}/>
              <Divider className={classes.divider}/>
            </div>
